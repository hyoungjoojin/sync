...
--changeset skkil:00005-oauth2-authorization-server-schema

-- Spring Authorization Server 7.0.2 표준 스키마.
-- 원본: spring-security-oauth2-authorization-server jar 내부의
-- oauth2-registered-client-schema.sql / oauth2-authorization-schema.sql /
-- oauth2-authorization-consent-schema.sql 를 그대로 옮기되, 해당 파일들이 지시하는
-- PostgreSQL 변환(blob -> text, timestamp -> timestamptz)만 적용했다.
-- 이 세 테이블의 구조는 프레임워크가 소유하므로 임의로 컬럼을 추가하거나 변경하지 않는다.
