...
CREATE TABLE media_files (
    ...
    CONSTRAINT chk_media_files_media_type CHECK (
        media_type IN (
            'IMAGE_APNG',
            'IMAGE_AVIF',
            'IMAGE_BMP',
...
CREATE TABLE projects (
    ...
    join_policy VARCHAR(50) NOT NULL DEFAULT 'INVITE',
    follower_count BIGINT NOT NULL DEFAULT 0,
    icon_media_id BIGINT,
    FOREIGN KEY(icon_media_id) REFERENCES media_files (id) ON DELETE
    SET
        NULL,
        CONSTRAINT chk_projects_private_requires_invite CHECK (
            is_public
            OR join_policy = 'INVITE'
        )
);
...
CREATE TABLE posts (
    ...
    SET
        NULL,
        FOREIGN KEY(cover_media_id) REFERENCES media_files (id) ON DELETE
    SET
        NULL
);
...
CREATE UNIQUE INDEX idx_tags_name_global ON tags (name)
WHERE
    project_id IS NULL;

CREATE TABLE tag_follow_relationships (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    follower_id BIGINT NOT NULL,
...
CREATE INDEX idx_tag_follow_relationships_tag ON tag_follow_relationships(tag_id);
...
CREATE TABLE post_references (
    ...
    UNIQUE (source_post_id, referenced_post_id),
    CONSTRAINT chk_post_references_no_self CHECK (source_post_id <> referenced_post_id)
);
...
CREATE TABLE post_series_posts (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    post_series_id BIGINT NOT NULL,
    post_id BIGINT NOT NULL,
    position INTEGER NOT NULL,
    FOREIGN KEY(post_series_id) REFERENCES post_series (id) ON DELETE CASCADE,
    FOREIGN KEY(post_id) REFERENCES posts (id) ON DELETE CASCADE,
    UNIQUE (post_id),
    UNIQUE (post_series_id, position) DEFERRABLE INITIALLY DEFERRED
);
...
CREATE INDEX idx_posts_personal_status_created_id ON posts(STATUS, created_at DESC, id DESC)
WHERE
    project_id IS NULL;
...
CREATE INDEX idx_collection_posts_collection_created_id ON collection_posts (collection_id, created_at, id);

CREATE INDEX idx_post_series_creator ON post_series(creator_id);

CREATE INDEX idx_post_series_project ON post_series(project_id);

CREATE INDEX idx_post_series_posts_position ON post_series_posts (post_series_id, position, id);

CREATE INDEX idx_post_references_source ON post_references(source_post_id, sort_order);

CREATE INDEX idx_post_references_referenced ON post_references(referenced_post_id);
